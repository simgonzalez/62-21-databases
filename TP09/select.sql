...
SELECT per_nom, per_prenom
FROM heg_personne
WHERE LOWER(per_prenom) = 'alain'
   OR LOWER(per_nom) = 'alain';
...
SELECT com_nom, com_lieu, com_date
...
SELECT per_prenom || ' ' || per_nom AS "Nom Complet"
FROM heg_personne;
...
SELECT 'Le ' || com_date || ' a lieu à ' || com_lieu || ' (' || com_ville || ') la compétition : ' ||
       com_nom AS "Compétition"
FROM heg_competition
ORDER BY com_date;
...
SELECT per_prenom, per_nom, clu_nom
FROM heg_personne
...
SELECT clu_nom, per_nom, per_prenom
FROM heg_club
...
SELECT DISTINCT per_prenom, per_nom
FROM heg_personne
         JOIN heg_participe ON per_no = par_per_no;
...
SELECT per_prenom, per_nom, clu_nom, per_nom AS "Président"
FROM heg_personne
         LEFT JOIN heg_club ON per_clu_no = clu_no
